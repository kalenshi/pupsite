...
class MemberForm(FlaskForm):
    ...
    """
    first_name = StringField(label="First Name", validators=[Length(min=2, max=50)])
    last_name = StringField(label="Last Name", validators=[Length(min=2, max=50)])
    email = EmailField(label="Email", validators=[Email()])
    verify_email = EmailField(
        label="Verify Email",
        validators=[EqualTo("email", "The emails must Match")]
    )
